...
open class ImagePickerController: UIViewController {
  ...
  let configuration1: ImagePickerConfiguration
  ...
  open lazy var galleryView: ImageGalleryView = { [unowned self] in
    let galleryView = ImageGalleryView(configuration: self.configuration1)
  ...
  open lazy var bottomContainer: BottomContainerView = { [unowned self] in
      let view = BottomContainerView(configuration: configuration1)
    view.backgroundColor = self.configuration1.bottomContainerColor
    view.delegate = self
  ...
  open lazy var topView: TopView = { [unowned self] in
    let view = TopView(configuration: self.configuration1)
  ...
  lazy var cameraController: CameraView = { [unowned self] in
    let controller = CameraView(configuration: self.configuration1)
  ...
  public override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
    self.configuration1 = ImagePickerConfiguration()
  ...
  open override func viewDidLoad() {
    ...
    view.backgroundColor = configuration1.mainColor
  ...
  open override func viewWillAppear(_ animated: Bool) {
    ...
    if configuration1.managesAudioSession {
  ...
  func presentAskPermissionAlert() {
    let alertController = UIAlertController(title: configuration1.requestPermissionTitle, message: configuration1.requestPermissionMessage, preferredStyle: .alert)

    let alertAction = UIAlertAction(title: configuration1.OKButtonTitle, style: .default) { _ in
      if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
        UIApplication.shared.openURL(settingsURL)
      }
    }

    let cancelAction = UIAlertAction(title: configuration1.cancelButtonTitle, style: .cancel) { _ in
  ...
  @objc func volumeChanged(_ notification: Notification) {
    guard configuration1.allowVolumeButtonsToTakePicture,
  ...
  @objc func adjustButtonTitle(_ notification: Notification) {
    ...
    let title = !sender.assets.isEmpty ?
      configuration1.doneButtonTitle : configuration1.cancelButtonTitle
  ...
  func enableGestures(_ enabled: Bool) {
    ...
    topView.rotateCamera.isEnabled = configuration1.canRotateCamera
  }
  ...
  fileprivate func takePicture() {
    ...
    if configuration1.collapseCollectionViewWhileShot {
...
extension ImagePickerController: CameraViewDelegate {
  ...
  func setFlashButtonHidden(_ hidden: Bool) {
    if configuration1.flashButtonAlwaysHidden {
  ...
  func imageToLibrary() {
    ...
    galleryView.fetchPhotos {
      ...
      if self.configuration1.allowMultiplePhotoSelection == false {
  ...
  func applyOrientationTransforms() {
    let rotate = configuration1.rotationTransform
